 ...
 *
 * @RestResource(
 *   id = "entity_legal_check",
 *   label = @Translation("Entity legal Check"),
 *   uri_paths = {
 *     "canonical" = "/legacy/check/{id}",
 *     "https://www.drupal.org/link-relations/create" = "/legacy/check"
 *   }
 * )
 */
class EntityLegalCheck extends ResourceBase {
  ...
  public function get($entity) {
    ...
    if ($document) {
      ...
      // Get acceptance by user and version.
      ...
      // Reset first key array.
      $data = reset($data);
      if (empty($data)) {
        $data = [
          "Message" => t("You need to accept the terms"),
          "status" => FALSE
        ];
      }
    }
    else {
      $data = ['Message' => t('The legal document is not correct')];
    }
